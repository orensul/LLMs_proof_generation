...
#
# Override with environment variables or --api_base / --api_key CLI flags.
...
# ---------------------------------------------------------------------------
#  Gemini model call  (OpenAI-compatible API)
# ---------------------------------------------------------------------------

def call_gemini(model, messages, temperature=0.7, max_tokens=16384, max_retries=5):
    """Call a Gemini model via Google's OpenAI-compatible API endpoint.

    Uses the endpoint at generativelanguage.googleapis.com/v1beta/openai
    which supports the standard OpenAI chat/completions format.
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(f"[Attempt {attempt + 1}] Error calling Gemini model: {e}")

        sleep_time = 2 ** attempt  # Exponential backoff
        print(f"Retrying in {sleep_time} seconds...")
        time.sleep(sleep_time)

    raise RuntimeError(f"call_gemini failed after {max_retries} retries.")


def gemini_response(messages, model_name):
    """Thin wrapper matching the gpt_response() signature."""
    return call_gemini(model=model_name, messages=messages)
...
def generate_and_verify(args, gdl_relevant_theorems, similar_problems, problem2, run_id):
    ...
    file_path = f"results/level_{problem2.level}/gemini/variant_{args.variant}_model_{args.model_name}_problem_{problem2.id}_run_{run_id}_to_verify.txt"
    attempts = 0
    verifier_result = ""
    resp = ""
    retries_messages = []
    ...
    while attempts < MAX_RETRIES_IN_RUN:
        ...
        resp = gemini_response(messages, args.model_name)
...
def run(args, problem2_id, problems, run_id):
    ...
    output_path = f"results/level_{problem2.level}/gemini/variant_{args.variant}_model_{args.model_name}_problem_{problem2.id}_run_{run_id}.txt"
