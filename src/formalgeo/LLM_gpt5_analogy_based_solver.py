...
# Uses the openai SDK (v0.28 here, same as LLM_analogy_based_solver.py).
# GPT-5 is a reasoning model: accepts a top-level `reasoning_effort` param
# ("minimal" | "low" | "medium" | "high"; default = "medium"),
# does NOT accept `temperature`, and uses `max_completion_tokens`.
#
# Put your key in the OPENAI_API_KEY env var, the variable below,
# or pass --api_key on the CLI.
...
chosen_problems_by_level = {
    5: [5440, 437] #  2795, 1168, 2677, 380, 944, 2940],
...
# ---------------------------------------------------------------------------
#  GPT-5 model call (OpenAI Chat Completions API)
# ---------------------------------------------------------------------------

def call_gpt5(model, messages, reasoning_effort="medium",
              max_completion_tokens=65536, max_retries=5):
    """Call an OpenAI GPT-5 reasoning model via the Chat Completions API.

    Mirrors the structure of `call_gpt_o1` in LLM_analogy_based_solver.py.
    GPT-5 reasoning model specifics:
      - `reasoning_effort` is top-level; "minimal" | "low" | "medium" | "high".
      - `temperature` is NOT supported.
      - Uses `max_completion_tokens` (NOT `max_tokens`); the budget includes
        invisible reasoning tokens, so keep it generous.
    """
    for attempt in range(max_retries):
        try:
            ...
            print(f"[Attempt {attempt + 1}] Empty response from GPT-5 "
    ...
    raise RuntimeError(f"call_gpt5 failed after {max_retries} retries.")


def gpt5_response(messages, model_name, reasoning_effort="medium"):
    """Thin wrapper matching the gpt_response()/gemini_response() signature."""
    return call_gpt5(model=model_name, messages=messages,
                     reasoning_effort=reasoning_effort)
...
def convert_json_list_to_custom_format(json_list):
    ...
    for index, item in enumerate(json_list, start=1):
        ...
        formatted_string = f"{index}; {theorem}; {premise}; {conclusion_str}"
...
def generate_and_verify(args, gdl_relevant_theorems, similar_problems, problem2, run_id):
    ...
    file_path = f"results/level_{problem2.level}/gpt5/variant_{args.variant}_model_{args.model_name}_problem_{problem2.id}_run_{run_id}_to_verify.txt"
    attempts = 0
    verifier_result = ""
    resp = ""
    retries_messages = []
    ...
    while attempts < MAX_RETRIES_IN_RUN:
        ...
        resp = gpt5_response(messages, args.model_name, reasoning_effort=args.reasoning_effort)
...
def run(args, problem2_id, problems, run_id):
    ...
    output_path = f"results/level_{problem2.level}/gpt5/variant_{args.variant}_model_{args.model_name}_problem_{problem2.id}_run_{run_id}.txt"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name", dest="model_name", type=str, default="gpt-5")
    parser.add_argument("--reasoning_effort", dest="reasoning_effort", type=str, default="medium",
                        choices=["minimal", "low", "medium", "high"],
                        help="GPT-5 reasoning effort (default: medium)")
    ...
    # Allow CLI arg to override the API key
    if args.api_key:
